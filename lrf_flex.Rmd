...
invest_df<- rreo %>% filter%>% filter(startsWith (rreo$conta, "INVES"), startsWith (rreo$coluna,"DESPESAS PAGAS ATÉ"), !startsWith (rreo$cod_conta,"InvestimentosIntra")) %>% group_by(instituicao, uf, exercicio, coluna, conta, periodo, periodicidade)  %>% summarise(valor = sum(valor))  %>% mutate (cores = if_else(valor > 5 , "blue","red"))
...
desp_pessoal_plot <- ggplot(pessoal_todos, aes(x=exercicio, y=valor, color = cores)) +
  ...
  geom_hline(yintercept=49, colour = "red", width=0.1) +
...
pessoal_uf_df<- dcl_df %>% filter (uf == "RJ")
pessoal_uf_df_line<- dcl_df %>% filter (uf == "RJ") %>% select(-cores)
pessoal_todos <-dcl_df %>% filter (uf != "RJ")
...
desp_pessoal_plot <- ggplot(pessoal_todos, aes(x=exercicio, y=valor, color = cores)) +
  ...
  geom_line(data = pessoal_uf_df_line,  inherit.aes =FALSE, aes(x=exercicio, y=valor,text=paste("UF: ", uf)), width=0.25, alpha=0.5)+
  geom_hline(yintercept=200, colour = "red", width=0.1) +
  #geom_hline(yintercept=44.5, colour = "orange", width=0.1) +
  geom_smooth(method = "auto", inherit.aes =FALSE,aes(x=exercicio, y=valor) )+
  theme(axis.text.x = element_text( hjust = 0.1)) +
  labs(title = "série histórica do % de comprometimento da Receita Corrente Líquida das UFs com a Despesa de Pessoal",
...
pessoal_uf_df<- invest_df %>% filter (uf == "RJ")
pessoal_uf_df_line<- invest_df %>% filter (uf == "RJ") %>% select(-cores)
pessoal_todos <-invest_df %>% filter (uf != "RJ")
...
desp_pessoal_plot <- ggplot(pessoal_df, aes(x=exercicio, y=valor,  color = cores)) +
  geom_jitter( aes(text=paste("UF: ", uf)), width=0.25, alpha=0.5, shape =1) +
  #geom_line(data = pessoal_uf_df_line, aes(x=exercicio, y=valor,text=paste("UF: ", uf)), width=0.25, alpha=0.5)+
  geom_point(data = pessoal_uf_df, aes(x=exercicio, y=valor,  color = cores,text=paste("UF: ", uf), size =1 ))+
  geom_hline(yintercept=49, colour = "red", width=0.1) +
  geom_hline(yintercept=44.5, colour = "orange", width=0.1) +
...
desp_pessoal_plot <- ggplot(pessoal_df, aes(x=exercicio, y=valor)) +
  geom_jitter( aes(text=paste("UF: ", uf)), width=0.25, alpha=0.5, shape =1)+
  geom_line(data = pessoal_uf_df_line, aes(x=exercicio, y=valor,text=paste("UF: ", uf)), width=0.25, alpha=0.5)+
  geom_point(data = pessoal_uf_df, aes(x=exercicio, y=valor, color = cores ,text=paste("UF: ", uf), size =1 ))+
  geom_hline(yintercept=49, colour = "red", width=0.1) +
